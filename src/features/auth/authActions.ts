...
export const userLogin = createAsyncThunk(
  ...
  async (params: LoginParamsForm, { rejectWithValue }) => {
    try {
      ...
      params.rememberMe
        ? localStorage.setItem(STORE_USER_INFO_KEY, data.user_name)
        : sessionStorage.setItem(STORE_USER_INFO_KEY, data.user_name);
      return data;
    } catch (error: any) {
        ...
        if (error.response && error.response.data.message) {
          ...
          switch (error.response.status) {
            case 422:
              ...
              break;
            case 500:
              alert("problems happened on the server!");
              break;
            default:
              alert("Unexpected error happened on login.");
              break;
          }
...
export const testAuthApi = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(`${APISERVER_URL}/test/v1/echo`, {
        withCredentials: true,
        params: {
          message: "hello",
        },
